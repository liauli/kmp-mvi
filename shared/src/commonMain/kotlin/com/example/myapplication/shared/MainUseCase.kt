...
import com.example.myapplication.shared.mvi.UseCase
    ...
    private val actionProcessor = { incomingFlow: Flow<MainAction> ->
        ...
        val actionInitialLoad = { actionFlow: Flow<MainAction.InitialLoadAction> ->
            actionFlow.flatMapConcat {
                flow<MainResult> {
                    emitAll(fetchVideo.execute().map { MainResult.LoadResult.Success(it?.play_list?: emptyList()) })
                }.catch {
    ...
    private val reducer = { resultFlow: Flow<MainResult> ->
        resultFlow.scan(MainState()) { previousState, result ->
            when (result) {
                ...
                is MainResult.LoadResult.Success -> previousState.copy(isLoading = false, data = result.video)
                MainResult.LoadResult.Failed -> previousState.copy(isLoading = false, isError = true)
            }
        }
    }
